...
def _run_engine(
    ...
    # A packaged build or local repo with assets ships the layout model so first run needs no network.
    bundled_model = os.environ.get("PDF_TRANSLATE_MODEL")
    if not bundled_model:
        local_model = SKILL_ROOT / "app" / "assets" / "doclayout.onnx"
        if local_model.is_file():
            bundled_model = str(local_model)
    local_font = SKILL_ROOT / "app" / "assets" / "GoNotoKurrent-Regular.ttf"
    if local_font.is_file():
        os.environ.setdefault("NOTO_FONT_PATH", str(local_font))

    model = _layout_model(bundled_model)
...
def inspect_pdf(path: Path) -> dict:
    """Return metadata about a PDF file: page count, size, and scan status."""
    source = Path(path).resolve()
    if not source.is_file():
        raise TranslationError(f"File does not exist: {source}")
